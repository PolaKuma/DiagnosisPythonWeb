...
import json
from .models import Patient, readBook
import jwt
import xlrd2
import time
...
class BooksManage:
    ...
    def listBooks(self, request):
        ...
        res = Patient.listBooks(pagenum, pagesize, key, value)
    ...
    def updateBooks(self, request):
        ...
        uid = newdata['id']
        # 去更新
        res = Patient.updateBooks(uid, newdata)
        return JsonResponse(res)

    # # 删除图书
    def deleteBooks(self, request):
        data = request.pd.get('id')
        res = Patient.deleteBooks(data)
        return JsonResponse(res)
